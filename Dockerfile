...
RUN tar -zxf euphoria-2-genesis.tar.gz
RUN mv euphoria-2-genesis.json $HOME/.bdjuno/genesis.json

RUN rm -rf $HOME/.bdjuno/config.yaml
COPY --from=builder /go/src/github.com/forbole/bdjuno/docker/bdjuno /root/.bdjuno
## RUN bdjuno parse genesis-file --genesis-file-path $HOME/.bdjuno/genesis.json
